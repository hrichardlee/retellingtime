...
# this should be in a common module
def _make_logger():
...
class Timeline(models.Model):
    ...
    class Meta:
        index_together = [
            ["is_valid", "sort_order_title"],
            ["is_valid", "highlighted", "sort_order_title"]
        ]
    ...
    def get_events(self):
        if self.orig_titles:
            ...
            (self.first_and_last, self.errors, self.fewer_than_threshold) = \
                wikipediaprocess.get_errors(raw_events, _event_threshold)
    ...
    def combine_events(cls, event_lists):
        return sorted([event for event_list in event_lists for event in event_list],
    ...
    def populate_from_search(cls):
        _logger.info('Searching wikipedia')
        timeline_titles = wikipedia.search_all('intitle:"timeline of"')
        _logger.info('Found {} pages'.format(len(timeline_titles)))
        for title in timeline_titles:
            if 'space exploration' in title.lower():
                _logger.info('Skipping {}'.format(title))
                continue
            _logger.info('Processing {}'.format(title))
            try:
                cls.process_wikipedia_page(title)
            except Exception:
                _logger.exception('Error in {}'.format(title))
