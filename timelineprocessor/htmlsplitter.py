...
class HtmlSplitter:
    """Initialize with an html string, and then call get_span to get the html strings that
    correspond to text-only strings."""
    ...
    def __init__(self, html_string):
        ...
        if len(soup.contents) > 1:
            ...
            for r, prevr in zip(top_level_ranges,
                                [{'el': None, 'range': (0, 0)}] + top_level_ranges):
                if not r['range']:
    ...
    def _get_text_indices(self, soup):
        """Given a soup, returns a list of ranges = [{range: (a, b), el: NavigableString},
        ...] where each bs4.NavigableString in the original soup corresponds to an item in the
        list. The range that is associated with each NavigableString represents the range in the
        text-only string of the original soup that corresponds to that NavigableString"""
    ...
    def _get_applicable_ranges(self, ranges, start, end):
        """Given a list of ranges = [{range: (a, b), el: soup}, ...], and (start, end), returns a
        list of ranges that overlap with (start, end). Assumes that ranges is sorted ascending by
        range and is contiguous."""
        applicable_ranges = []
        for r in ranges:
            if r['range'][1] > start:
                if r['range'][0] < end:
    ...
    def _modify_ranges(self, ranges, start=None, end=None):
        """Given a list of ranges, and (start, end), modifies the elements referenced by ranges
        such that only the text corresponding to (start, end) remains. If start or end are None,
        then assumes that start is 0 and end is infinity. Assumes ranges is non-empty"""
        ...
        if start is not None: remove_siblings(ranges[0]['el'], 'prev')
        if end is not None: remove_siblings(ranges[-1]['el'], 'next')
        ...
        if len(ranges) == 1 and start and end:
            ...
            # this causes somewhat weird behavior when there's an image or something at the end
            # of an element
            if end:
    ...
    def get_span(self, start, end):
        """Given indices (start, end) in the pure-text version of the htmlString this object is
        initialized with, returns the html string that corresponds to the specified text string"""
        ...
        top_level_ranges = [{'el': BeautifulSoup(unicode(r['el'])), 'range': r['range']} for r in
                            self._get_applicable_ranges(self._top_level_ranges, start, end)]
