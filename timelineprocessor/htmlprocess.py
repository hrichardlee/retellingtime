...
_ignore_classes = {
    'thumb', 'toc', 'navbox', 'vertical-navbox'
}


def html_to_string_blocks(html):
    """Given an html element as a BeautifulSoup, returns a list of string blocks. Each string
    block represents a section in the html demarcated by a header element. Each string block is {
    heading, lines: [{line_type, line}]}. Lines is a list of objects. line_type = LineTypes.table
    indicates that line is a BeautifulSoup table element. line_type = LineTypes.line indicates
    that the line is a string where each block element in html is a string. Heading is a list
    where the nth element represents the nth subheading applicable to the current section. Empty
    ...
    for el in html.children:
        ...
        if el.name in _header_elements:
            ...
            curr_heading.append(el.find(class_='mw-headline').text
                                if el.find(class_='mw-headline')
                                else el.get_text())
...
def string_blocks_to_events(string_blocks, p=None):
    """Given a set of string blocks (as produced by html_to_string_blocks, expects that all
    strings are non-empty), returns a list of timeline events. A timeline event is {date: number,
    date_string: string, content: string}"""
        ...
        if section_test(string_block['heading'][0]):
            ...
            for h in string_block['heading']:
                ...
                if parse:
                    ...
                    base_date_string = parse[1]  # it's possible this should be +=
            base_date_string_lower = base_date_string.lower().strip()
            ...
            if base_date.start_year() is not None:
                delta_minus = 10
                delta_plus = 20
                ...
                base_date_range = (
                    base_date.start_year() - delta_minus, base_date.start_year() + delta_plus)

            for line in string_block['lines']:
                if line['line_type'] == LineTypes.line:
                    parse = parse_date_html(line['line'])
                    # if we can parse a date, and the parsed date matches the current heading,
                    # create a new event
                    if parse and \
                        ((not base_date_range) or
                            (parse[0].start_year() is None) or
                            (base_date_string_lower == 'antiquity') or
                            (('prior' in base_date_string_lower or
                              'before' in base_date_string_lower or
                              base_date_range[0] <= parse[0].start_year()) and
                             ('after' in base_date_string_lower or
                              parse[0].start_year() <= base_date_range[1])) or
                            (TimelineDate.can_combine_as_day(base_date, parse[0]))):

                        _close_event(events, curr_event)
                        date = parse[0]
                        if date.start_year() is None and prev_date:
                    ...
                    # use the base_date
                    elif base_date.start_year() is not None:
...
def _bs_fix_brs(soup):
    """This function only works on soups (if it is given a tag, it will do nothing). When
    BeautifulSoup parses a <br> tag, it makes a closing <br /> tag somewhere later in the
    document. This function fixes this problem."""
    if soup.new_tag:
...
def _bs_inner_html(soup):
    """Gets the inner html (almost) of a BeautifulSoup element. It is slightly different than the
    javascript function because it drops all comments and comment-like items"""
...
def _lines_from_html(html):
    return (line['line'] for lines in
            (block['lines'] for block in html_to_string_blocks(html))
            for line in lines
            if line['line_type'] == LineTypes.line)


def _table_to_events(table, base_date, p=None):
    """Given a table html element as a BeautifulSoup, returns a list of events"""
    ...
    if date_col_index is not None and year_col_index is None:
        year_col_index = date_col_index
        date_col_index = None
    if year_col_index is None and date_col_index is None:
        # just try using the first column. could be a bit smarter about giving
        # up early to save some cycles...
        year_col_index = 0

    if year_col_index is not None or date_col_index is not None:
        ...
        for row in table.find_all('tr'):
            ...
            if len(cells) == 1:
                ...
                if extract:
                    date = extract[0]
                    date_string = extract[1]
                    if date_col_index is not None and len(cells) > date_col_index:
                    ...
                    date = TimelineDate.combine(base_date, date)
                    content_cells = [cell for (i, cell) in enumerate(cells) if
                                     i != year_col_index and i != date_col_index]
                    if p['keep_row_together']:
                        ...
                        rowspan_cells = [cell for cell in content_cells if
                                         get_rowspan(cell) is not None]
...
def clean_events(events):
    """Takes raw events and replaces any problematic elements. Modifies events in place."""
    for e in events:
