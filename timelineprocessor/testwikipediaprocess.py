...
import wikipedia
from timelineprocessor.testwikidata import *
import copy
...
class TestHtmlToStringBlocks(unittest.TestCase):
    def test_one(self):
        ...
        strings = htmlprocess.html_to_string_blocks(html)
...
class TestStringBlocksToEvents(unittest.TestCase):
    ...
    def test_one(self):
        evs = htmlprocess.string_blocks_to_events(stringblocksone)
        self.p(evs)
        self.assertEqual(len(evs), 6)

    def test_two(self):
        evs = htmlprocess.string_blocks_to_events(stringblockstwo)
        self.p(evs)
        self.assertEqual(len(evs), 1)

    def test_three(self):
        article = BeautifulSoup(html_particlephysics)
        evs = htmlprocess.string_blocks_to_events(htmlprocess.html_to_string_blocks(article))
        self.p(evs)
...
class TestPerf(unittest.TestCase):
    def test_time_modern_history(self):
        cProfile.runctx('wikipediaprocess.wp_page_to_json("Timeline_of_modern_history")',
                        globals(), locals())

    def test_time_modern_history_separate(self):
        cProfile.runctx(
            'wikipediaprocess.wp_page_to_json("Timeline_of_modern_history", separate = True)',
            globals(), locals())
