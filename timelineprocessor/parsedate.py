...
class TimePoint:
    ...
    @classmethod
    def combine(cls, a, b):
        """Given TimePoints a and b, returns a new TimePoint that self takes on b's attributes,
        except if b does not have that attribute, in which case a's are taken."""
        ...
        if b.month is not None: month = b.month
        else: month = a.month
        if b.day is not None: day = b.day
        else: day = a.day
        return cls(year, month, day, year_approx=year_approx)


class TimelineDate:
    """Represents either a year or a range of years. simple_year returns the best single number
    approximation of the time of the event"""
    ...
    def can_combine_as_day(cls, a, b):
        return not a.end and not b.end \
               and a.start.year is not None and a.start.month is not None and a.start.day is None \
               and b.start.year is not None and b.start.month is None and b.start.day is None \
               and 1 <= b.start.year <= 31

    @classmethod
    def combine(cls, a, b):
        """Given two TimelineDates a and b, returns a new TimelineDate. The result's start will
        be a's start combined with b's start and the result's end will be a's end combined with
        b's end. (It doesn't really make sense for a to have an end, but we're not going to deal
        with that here.)

        The only exception is that if a and b both do not have ends, and a has year and month but
        not day and b has only year and self, and its value for year is between 1 and 31 the
        result will be combining a with b but interpreting b to only have a day value."""
...
def month_to_num(text):
    """This takes a month node name as defined in the date grammar and converts it to a number"""
...
def _possible_texts(text):
    """Given a string, returns a generator of strings which are all possible date_text strings
    for the given input, anchored at the first character"""
    # warning: this will only work as long as there are no cases where concatenating multiple words
    # in date_grammar_words could form another valid word. ad/bc/ce/bce are an exception,
    # they will work fine
    ...
    for i, t in enumerate(tokens):
        if _valid_nonwords_re.search(t) is None and t not in _date_grammar_words_set:
            tokens = tokens[:i]
            break
    prev_is_break_char = True
    # generate all possible strings
    for i, t in reversed(list(enumerate(tokens))):
        if t and _whitespace_re.search(t) is None and prev_is_break_char:
            yield ''.join(tokens[:i + 1])
        if t:
            # this is to make sure we only end the date string on valid
            # characters
            prev_is_break_char = _valid_end_char_re.search(t[0]) is not None
        # if t is an empty string, don't change prev_is_break_char


def parse_date_text(text):
    """Parses a date from some text, returns (TimelineDate, index) where text[:index] is the text
    determined to be the date. If no date can be found, returns None. Assumes that date is at the
    beginning of the string	with no superfluous characters."""
    text = text.lower()
    ...
    def yadyymymd(yad):  # returns TimePoint
        ...
        if daynum is not None: yeartp.day = daynum
        return yeartp

    def year(year):  # returns TimePoint
        # because we are using astronomical years in which x BC is stored as (-x + 1)
        if year.node == 'YBC': return -num(year[0]) + 1
    ...
    def monthdayrange(r):  # returns TimelineDate
        ...
        if r[0].node == 'DAY':
            first = TimePoint(second.year, second.month, int(numstr(r[0])),
                              year_approx=second.year_approx)
        elif r[0].node == 'MONTH':
    ...
    def monthdayyearrange(r):  # returns TimelineDate
        ...
        return TimelineDate(
            TimePoint(yeartp.year, monthdaytp.month, monthdaytp.day,
                      year_approx=yeartp.year_approx),
            TimePoint(yeartp.year, monthdaytp.month, int(numstr(r[2])),
                      year_approx=yeartp.year_approx))
...
def parse_date_html(html_string):
    """Takes a string that contains html, and returns (date, date_string, content) as a tuple.
    For now, date is an int that represents the year. Negative numbers are B.C. and positive are
    A.D. years. If there is no date that can be parsed, returns None."""

    # preprocess to add newlines after <br />, or else get_text smushes things together
